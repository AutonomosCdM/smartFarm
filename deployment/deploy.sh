...
# 10.5. Install Excel processing dependencies
echo -e "${YELLOW}📦 Installing Excel processing dependencies...${NC}"
if [ -f "./deployment/install-excel-dependencies.sh" ]; then
    bash ./deployment/install-excel-dependencies.sh open-webui
else
    echo -e "${YELLOW}⚠️  Excel dependency installer not found, installing manually...${NC}"
    docker exec open-webui pip install llama-index-llms-groq llama-index-embeddings-openai --quiet || true
fi
echo -e "${GREEN}✅ Excel dependencies installed${NC}"
